...
try:  # Optional dependency already available in the environment.
...
def _build_client() -> Groq | None:  # type: ignore[valid-type]
...
def answer_explanation_chat(
    *,
    ...
    try:
        ...
        reply = normalize_text(completion.choices[0].message.content if completion.choices else "")
        if not reply:
            reply = _fallback_reply(analysis=analysis, message=normalized_message, language=language)
